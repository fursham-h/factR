...
Method to download the above GTF files is described in the  ["Importing and inspecting GTF data"](#importing-and-inspecting-gtf-data) section below.  
...
```{r importGTF, warning=FALSE}  
library(factRdata)
selected.data <- "sc"  
input.file <- system.file("extdata", paste0(selected.data, "_merged.gtf.gz"), package = "factRdata")

custom.gtf <- importGTF(input.file)  
```  
This should import sc_merged.gtf.gz, and we will use this input in the walk-through below. To try bulk_merged.gtf.gz or lr_merged.gtf.gz, simply change "sc" to "bulk" or "lr" in the selected.data variable. Users may import their own GTF file by specifying the path to such file in the `importGTF` function. 
...
2. Downloading from a GENCODE database (ref.gtf generated from this method will be used for the remaining of the workflow) 
...
1. Using *BSgenome*  (Mmusculus generated from this method will be used for the remaining of the workflow)
