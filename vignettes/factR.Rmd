...
Users can quickly visualize the exon structure of these query transcripts using the `plotGTFtranscripts` tool, which is essentially a convenient wrapper to plot transcripts using the *wiggleplotr* package:
```{r viewquery}
...
```{r matchseqlvls}
# matching seqlevels
matched_query_gtf <- matchSeqLevels(query_gtf, Mmusculus)

head(matched_query_gtf)
```
Note: `matchSeqLevels` will drop any unmatched seqlevels from query 

*factR* also comes with the function `has_consistentSeqlevels` to test if two or more objects have consistent seqlevels. This function returns a logical value of FALSE if input objects have inconsistent seqlevels and recommends a `matchSeqLevels` function to run.
```{r testseqlevels}
...
Custom-built GTF annotation may contain generic **gene_id** and **gene_name** metadata (e.g. 'MSTRG1'), depending on how it was constructed. Users who wish to fill these metadata using a reference annotation may run the `matchGeneMeta` function:
```{r matchgenemeta}
# matching gene metadata
matched_query_gtf <- matchGeneMeta(matched_query_gtf, ref_gtf)

head(matched_query_gtf)
```

By default, this function will match query transcripts to reference by overlapping coordinates. Users may improve the accuracy of this matching function by providing values to **primary_gene_id** and **secondary_gene_id** arguments. See ?matchGeneMeta for detailed description.
...
User can visualize the coding structure of each transcripts using the `plotGTFtranscripts` tool, which is essentially a convenient wrapper to plot transcripts using the *wiggleplotr* package:
```{r viewCDS}
plotGTFtranscripts(new_query_gtf)
```
...
Upon construction of coding segments on transcripts, users may be interested in determining the functional domains which is encoded within the coding sequence. To this end, users can utilize the `extractCDSfeature` function. This tool accepts a GTF GRanges object containing CDS information as input, together with a genome sequence object. After checking the translated protein sequence, `extractCDSfeature` will query the SuperFamily database for domain features by calling out the `hmmer` function from *bio3d* package. The function will output 3 output fields: **desc**, **evalue** and **nincluded**. 
```{r testfeature, eval=F}
extractCDSfeature(new_query_gtf, Mmusculus)
#> # A tibble: 4 x 4
...
More fields from the `hmmer` function can be requested by providing a list of output fields to the hmmer argument:
```{r testfeature2, eval=F}
extractCDSfeature(new_query_gtf, Mmusculus, hmmer = c('default', 'bitscore', 'pvalue'))
#> # A tibble: 4 x 6
...
This tool also currently support the identification of signal peptide by running the `signalHsmm` function from the *signalHsmm* package. This feature is not run by default, but can be initiated by setting the argument for **signalHsmm** to 'default' or any one of the supported output fields. Users can skip testing of any of these featyres by setting the **hmmer** or **signalHsmm** arguments to NULL
```{r testfeature3, eval=F}
extractCDSfeature(new_query_gtf, Mmusculus, hmmer = NULL, signalHsmm = 'default')
#> # A tibble: 4 x 2
#>   id          signalHsmm.sp_probability
#>   <chr>       <chr>                    
#> 1 transcript1 0.000574583738246925     
#> 2 transcript2 0.000574583738246925     
#> 3 transcript3 0.000574583738246925     
#> 4 transcript4 0.000574583738246925     
```
...
#> extractCDSfeature(new_query_gtf, Mmusculus, transcript_id %in% c('transcript1', 'transcript3'))
#>            id              hmmer.desc hmmer.evalue hmmer.nincluded
#> 1 transcript1 RNA-binding domain, RBD      2.7e-81               4
#> 2 transcript3 RNA-binding domain, RBD      1.3e-43               2
```
...
```{r testnmdfilter}
predictNMD(new_query_gtf, transcript_id == 'transcript3')
```
...
predictNMD(mm10_gtf, gene_name == 'Ptbp1')
#> # A tibble: 9 x 6
...
predictNMD(mm10_gtf, gene_name == 'Ptbp1', transcript_id == c('ENSMUST00000057343', 'ENSMUST00000095457'))
#> # A tibble: 2 x 6
#>   transcript         stop_to_lastEJ num_of_downEJs stop_to_downEJs threeUTRlength is_NMD
#>   <chr>                       <dbl>          <int> <chr>                    <dbl> <lgl> 
#> 1 ENSMUST00000057343            364              3 "69,286,364"               644 TRUE  
...
To find and annotate alternative-spliced segments within a GTF file, users can simply run `annotateAS`. This function accepts any GTF GRanges object with exon features and will return a GRanges object containing annotated alternative segments in a similar GTF format
```{r annotateAS}
annotateAS(new_query_gtf)
```
...
system.time({mm10_AS <- annotateAS(mm10_gtf)})
#>    user  system elapsed 
#>  21.352   0.076  21.427 
...
Users who wish to compare alternative segments between two or more transcripts can run the `compareAS` function. In contrast to `annotateAS`, this function accepts two or more GRanges object, each containing exon features for a particular transcript. Alternatively, users can provide the list of transcript exon features as a GRangesList object
```{r compareAS}
