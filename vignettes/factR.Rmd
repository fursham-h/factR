...
`factR` also comes with a function to test if two or more objects have consistent seqlevels. This function returns a logical value of FALSE if input objects have inconsistent seqlevels and recommends a `matchSeqLevels` function to run.
```{r testseqlevels}
...
This tool also currently support the identification of signal peptide by running the `signalHsmm` function from the `signalHsmm` package. This feature is not run by default, but can be initiated by setting the argument `signalHsmm` to 'default' or any one of the supported output fields. Users can skip testing of any of these featyres by setting the `hmmer` or `signalHsmm` arguments to NULL
