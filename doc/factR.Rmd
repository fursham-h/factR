...
The information-rich output from `predictNMD` and `predictDomains` can be processed to give meaningful summaries of the data. Here we show common operations that we perform on *factR* output. 
### summarising NMD data
We begin with processing the NMDprediction.out object. With the help of data manipulation tools from the tidyverse package, we can compute the number of NMD-sensitive and non NMD-sensitive transcripts from this list of newly-found RNAs:
...
Furthermore, the mean 3'UTR lengths of these two groups of transcripts can be easily determined:
...
Comparisons can be made between newly-discovered and known NMD transcripts. To determine the prevalence of NMD in these two groups of transcripts, we first have to predict NMD susceptibility on reference transcripts:
...
Next, we combine both dataframes and distinguish new from known transcripts:
```{r NMDanalysis1}
...
Lastly, the list of combined transcripts are categorised into 4 groups based on its source (“New” vs “Known”) and its NMD sensitivity, and the number of transcripts in each group is calculated:
...
Gain and loss of functional domains can profoundly alter a protein’s output function. Here, we will identify domains frequently added in newly-predicted proteins. We start by predicting protein domains on known reference proteins:
...
Next, we combine dataframes domains.out and domains.known and distinguish new from known proteins:
```{r domcombine}
domains.combine <- bind_rows(domains.known, domains.out) %>%
  mutate(group = ifelse(transcript %in% domains.out$transcript, "New", "Known")) 
```

To be able to group proteins by genes, we first have to append gene information to our combined data frame:
...
The next lines of code generate an ordered list of domains for each protein and retains unique domain structures:
```{r nestdomains}
domains.combine <- domains.combine %>%
  group_by(transcript) %>% 
  arrange(begin) %>%
  mutate(domain = list(description)) %>% 
  #nest(c(description,n)) %>% 
...
domains.combine.filtered <- domains.combine %>%
  group_by(gene_id) %>%
  mutate(anynew = ifelse("New" %in% group, TRUE, FALSE)) %>%
  ungroup() %>% 
  filter(anynew) %>% 
  select(-anynew)
```

Then, domains gained in new proteins are shortlisted:
```{r shortlistgains}
...
Top 10 frequently gained domains are identified using this set of code:
```{r top10domains}
...
The summarised data above can be presented as a bar chart:
```{r plotdomains}
domains.combine.top %>% 
...
We can list the first 10 genes with gain of C2H2 doamin:
```{r list10C2H2}
...
And view the domain architecture of new and known proteins in the gene "Zfp972":
```{r showZfp972}
