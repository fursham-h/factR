...
``` 
This should import sc_merged.gtf.gz, and we will use this input in the walk-through below. To try bulk_merged.gtf.gz or lr_merged.gtf.gz, simply change "sc" to "bulk" or "lr" in the selected.data variable. Users may import your own GTF file by typing the path to the GTF file into `importGTF` function.
...
The following command will calculate the total number of transcripts in the input transcriptome: 
```{r counttx} 
...
We then generate an ordered list of domains for each protein retaining unique domain structures: 
```{r nestdomains} 
domains.combine <- domains.combine %>% 
  group_by(transcript) %>%  
  arrange(begin) %>% 
