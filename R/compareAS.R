...
compareAS <- function(exons, ..., groupings = NULL) {
  ...
  if (length(list(...)) > 0) {
    ...
    if (!is(tx2, "GRanges")) {
      ...
      rlang::warn(sprintf(
        "`%s` is not a GRanges object. Intra-list comparison mode was executed",
        argnames[[2]]
      ))
    } else {
      ...
      if (!is(exons, "GRanges")) {
        if (is(exons, "GRangesList")) {
          exons <- exons[[1]]
          rlang::warn(sprintf(
            "GRangesList objects are not supported in pair-wise mode. First item in `%s` was used",
            argnames[[1]]
          ))
        } else {
          # error out
          rlang::abort(sprintf(
            "`%s` is not of class GRanges or GRangesList",
            argnames[[1]]
          ))
        }
      }
  ...
  if (is(exons, "GRanges")) {
    rlang::abort(sprintf(
      "%s has to be GRangesList in intraList mode",
      argnames[[1]]
    ))
  }
  ...
  if (length(exons) == 1) {
    rlang::abort(sprintf(
      "%s has to contain >1 items intraList mode",
      argnames[[1]]
    ))
  }
  ...
  if (!is.null(groupings)) {
    ...
    if (any(!groupings[[2]] %in% names(exons))) {
      ...
      rlang::warn(sprintf(
        "%s transcript(s) in `%s` have missing GRanges in `%s`. These were not analyzed",
